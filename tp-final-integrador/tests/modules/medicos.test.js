...
describe('Médicos - Integration Tests', () => {
    ...
    beforeEach(async () => {
        ...
        const [pacienteUserResult] = await pool.execute('INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', ['99999999', 'Perez', 'Pedro', 'pedro.perez@test.com', 'hash', '', ROLES.PACIENTE, 1]);
        const pacienteUserId = pacienteUserResult.insertId;
        patientToken = jwt.sign({ id: pacienteUserId, rol: ROLES.PACIENTE, documento: '99999999' }, JWT_SECRET);
        // 1. Crear Especialidad
        const [espResult] = await pool.execute('INSERT INTO especialidades (nombre, activo) VALUES (?, 1)', ['PEDIATRÍA']);
        espActivaId = espResult.insertId;
        const [espSinMedicosResult] = await pool.execute('INSERT INTO especialidades (nombre, activo) VALUES (?, 1)', ['CLÍNICA MÉDICA']);
        espSinMedicosId = espSinMedicosResult.insertId;
        const [espInactivaResult] = await pool.execute('INSERT INTO especialidades (nombre, activo) VALUES (?, 0)', ['TRAUMATOLOGÍA INACTIVA']);
        espInactivaId = espInactivaResult.insertId;
        // 2. Crear Usuario Médico
        const [userResult] = await pool.execute('INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', ['12345678', 'Gomez', 'Juan', 'juan.gomez@test.com', 'hash', '', 1, 1]);
        const userId = userResult.insertId;
        // 3. Crear Médico
        const [medicoResult] = await pool.execute('INSERT INTO medicos (id_usuario, id_especialidad, matricula, descripcion, valor_consulta) VALUES (?, ?, ?, ?, ?)', [userId, espActivaId, 5555, 'Test description', 5000.0]);
        medicoId = medicoResult.insertId;
        // 4. Crear Obras Sociales
        const [os1] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['OS 1', 'Desc 1', 10, 0, 1]);
        osActivaId1 = os1.insertId;
        const [os2] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['OS 2', 'Desc 2', 15, 0, 1]);
        osActivaId2 = os2.insertId;
        const [os3] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['OS 3', 'Desc 3', 5, 0, 0]);
    ...
    describe('POST /api/v1/medicos/:idMedico/obras-sociales', () => {
        it('debería asociar múltiples obras sociales exitosamente (201)', async () => {
            ...
            const [rows] = await pool.execute('SELECT * FROM medicos_obras_sociales WHERE id_medico = ?', [medicoId]);
            expect(rows).toHaveLength(2);
        });
        it('debería ser idempotente si ya existe una asociación (201/200)', async () => {
            // Pre-asociar OS 1
            await pool.execute('INSERT INTO medicos_obras_sociales (id_medico, id_obra_social, activo) VALUES (?, ?, 1)', [medicoId, osActivaId1]);
            const response = await request(app)
                .post(`/api/v1/medicos/${medicoId}/obras-sociales`)
                .set('Authorization', `Bearer ${adminToken}`)
                .send({ obrasSociales: [osActivaId1, osActivaId2] });
        ...
        it('debería manejar duplicados en el mismo request (idempotencia en el lote)', async () => {
            ...
            expect(response.body.data.asociadas).toContain(osActivaId1);
            const [rows] = await pool.execute('SELECT * FROM medicos_obras_sociales WHERE id_medico = ? AND id_obra_social = ?', [medicoId, osActivaId1]);
