...
describe('Usuarios Model - createPacienteUser', () => {
  ...
  it('debe crear un paciente exitosamente cuando existe una obra social "Particular"', async () => {
    ...
    const [osResult] = await pool.execute(
      ...
      ['Obra Social de Test', 'Particular Test', 0, 1, 1],
    );
    ...
    const [pacientes] = await pool.execute(
      'SELECT id_obra_social FROM pacientes WHERE id_usuario = ?',
      [createdUser.id],
    );
  ...
  it('debe lanzar un error si NO existe una obra social marcada como particular', async () => {
    ...
    await pool.execute(
      'INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)',
      ['Normal OS', 'Not particular', 0.1, 0, 1],
    );
    ...
    await expect(usuariosModel.createPacienteUser(newPaciente)).rejects.toThrow(
      'No se encontró la obra social "Particular" en el sistema.',
    );
