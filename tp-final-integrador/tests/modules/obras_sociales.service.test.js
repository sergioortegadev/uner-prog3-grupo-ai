...
describe('Obras Sociales - Unit Tests (Service)', () => {
    ...
    describe('createObraSocial()', () => {
        ...
        it('debería lanzar error si ya existe una obra social con ese nombre', async () => {
            ...
            await expect(obrasSocialesService.createObraSocial(nuevaObra)).rejects.toThrow('Ya existe una obra social con ese nombre');
        ...
        it('debería lanzar error con sugerencia de reactivación si la obra social existe pero está inactiva', async () => {
            ...
            await expect(obrasSocialesService.createObraSocial(nuevaObra)).rejects.toThrow("Ya existe la obra social 'Swiss Inactiva' pero se encuentra inactiva. Debería reactivarla.");
            expect(obrasSocialesModel.create).not.toHaveBeenCalled();
        });
    });
    ...
    describe('updateObraSocial()', () => {
        ...
        it('debería manejar la condición de carrera si el nombre se duplica entre el chequeo y la actualización', async () => {
            ...
            await expect(obrasSocialesService.updateObraSocial(id, updateData)).rejects.toThrow('Duplicate entry');
        });
        it('debería lanzar error si el nombre nuevo ya existe en otra obra social', async () => {
            ...
            await expect(obrasSocialesService.updateObraSocial(id, updateData)).rejects.toThrow('Ya existe otra obra social con ese nombre');
            expect(obrasSocialesModel.update).not.toHaveBeenCalled();
        });
        it('debería lanzar error si el nombre nuevo ya existe pero está inactivo en otra obra social', async () => {
            ...
            await expect(obrasSocialesService.updateObraSocial(id, updateData)).rejects.toThrow("Ya existe la obra social 'Inactivo' pero se encuentra inactiva. No puede usar este nombre.");
            expect(obrasSocialesModel.update).not.toHaveBeenCalled();
        });
