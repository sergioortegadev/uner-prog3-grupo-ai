...
import { DB_STATUS } from '../../src/constants/common.constants.js';
...
describe('Médicos Model - Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const [uRes] = await pool.execute(
      ...
      ['100', 'Doc', 'One', 'doc1@test.com', 'hash', '', ROLES.MEDICO, 1],
    );
  ...
  describe('findAll()', () => {
    it('debería retornar solo médicos con usuario activo', async () => {
      // Inactive doctor
      const [u2Res] = await pool.execute(
        'INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['200', 'Doc', 'Two', 'doc2@test.com', 'hash', '', ROLES.MEDICO, 0],
      );
      await pool.execute(
        'INSERT INTO medicos (id_usuario, id_especialidad, matricula, valor_consulta) VALUES (?, ?, ?, ?)',
        [u2Res.insertId, espId, 2002, 6000],
      );
  ...
  describe('Obras Sociales logic', () => {
    it('acceptsObraSocial debería retornar true si está asociada y activa', async () => {
      await pool.execute(
        ...
        [medicoId, osId, DB_STATUS.ACTIVE],
      );
  ...
  describe('assignObrasSociales()', () => {
    ...
    it('debería reactivar asociaciones inactivas', async () => {
      await pool.execute(
        'INSERT INTO medicos_obras_sociales (id_medico, id_obra_social, activo) VALUES (?, ?, ?)',
        [medicoId, osId, DB_STATUS.INACTIVE],
      );
  ...
  describe('updateEspecialidad()', () => {
    it('debería actualizar el campo id_especialidad', async () => {
      const [esp2Res] = await pool.execute(
        'INSERT INTO especialidades (nombre, activo) VALUES (?, 1)',
        ['OTRA'],
      );
