...
describe('Médicos Routes - Integration Tests', () => {
    ...
    beforeEach(async () => {
        await setupTestDB();
        // Create Specialty
        const [espRes] = await pool.execute('INSERT INTO especialidades (nombre, activo) VALUES (?, 1)', ['PEDIATRIA']);
        espId = espRes.insertId;
        // Create Obras Social
        const [osRes] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['OS 1', 'Desc', 10, 0, 1]);
        osId = osRes.insertId;
        // Create Users
        const createUser = async (doc, email, rol) => {
            const [res] = await pool.execute('INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [doc, 'Surname', 'Name', email, 'hash', '', rol, 1]);
            return {
                id: res.insertId,
        ...
        doctorId = d.id;
        // Create Médico
        const [mRes] = await pool.execute('INSERT INTO medicos (id_usuario, id_especialidad, matricula, valor_consulta) VALUES (?, ?, ?, ?)', [doctorId, espId, 1234, 5000]);
