...
describe('Obras Sociales - Integration Tests', () => {
    ...
    describe('POST /api/v1/obras-sociales', () => {
        ...
        it('debería retornar 409 si el nombre ya existe', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Duplicada', 'Test', 0.15, 0, 1]);
            const nuevaObra = {
                nombre: 'Duplicada',
                descripcion: 'Otra',
            };
        ...
        it('debería retornar 409 si el nombre existe pero está inactivo (sin reactivación automática)', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Inactiva', 'Vieja', 0.1, 0, 0]);
            const nuevaObra = {
                nombre: 'Inactiva',
        ...
        it('debería retornar 409 si el nombre ya existe con diferente case (case-insensitive)', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Swiss Medical', 'Test', 0.15, 0, 1]);
            const nuevaObra = {
    ...
    describe('PUT /api/v1/obras-sociales/:id', () => {
        it('debería actualizar los datos correctamente', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['A Actualizar', 'Test', 0.15, 0, 1]);
            ...
            const [rows] = await pool.execute('SELECT nombre FROM obras_sociales WHERE id_obra_social = ?', [id]);
        ...
        it('debería permitir actualizar una obra social inactiva (reactivación)', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Inactiva Update', 'Test', 0, 0]);
            const id = result.insertId;
            const response = await request(app)
                .put(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`)
            ...
            expect(response.status).toBe(200);
            const [rows] = await pool.execute('SELECT activo FROM obras_sociales WHERE id_obra_social = ?', [id]);
            expect(rows[0].activo).toBe(1);
        });
        it('debería retornar 409 al actualizar con un nombre que ya existe en otra obra social', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Existe', 'Test', 0.15, 0, 1]);
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Otra', 'Test', 0.15, 0, 1]);
            const id = result.insertId;
            const response = await request(app)
                .put(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`)
        ...
        it('debería permitir actualizar manteniendo el mismo nombre', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Mismo Nombre', 'Test', 0.15, 0, 1]);
            const id = result.insertId;
            const response = await request(app)
                .put(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`)
            ...
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
            const [rows] = await pool.execute('SELECT descripcion FROM obras_sociales WHERE id_obra_social = ?', [id]);
            expect(rows[0].descripcion).toBe('Nueva desc');
        });
        it('debería actualizar múltiples campos simultáneamente', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Multiples Campos', 'Test', 0.15, 0, 1]);
            const id = result.insertId;
            const response = await request(app)
                .put(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`)
            ...
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
            const [rows] = await pool.execute('SELECT nombre, descripcion, porcentaje_descuento, es_particular FROM obras_sociales WHERE id_obra_social = ?', [id]);
        ...
        it('debería retornar 422 si el porcentajeDescuento en PUT es inválido (> 1)', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Descuento Mal', 'Test', 0.15, 0, 1]);
            const id = result.insertId;
            const response = await request(app)
                .put(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`)
        ...
        it('debería retornar 422 si el nombre en PUT supera los 120 caracteres', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Nombre Largo Put', 'Test', 0.15, 0, 1]);
            const id = result.insertId;
            const response = await request(app)
                .put(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`)
        ...
        it('debería actualizar sólo la descripción', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Solo Desc', 'Test', 0.15, 0, 1]);
            const id = result.insertId;
            const response = await request(app)
                .put(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`)
        ...
        it('debería ignorar campos desconocidos en el body', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Ignora Desconocidos', 'Test', 0.15, 0, 1]);
            const id = result.insertId;
            const response = await request(app)
                .put(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`)
    ...
    describe('GET /api/v1/obras-sociales', () => {
        it('debería retornar la lista con metadatos de paginación', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['Swiss Medical', 'Prepaga', 0.15, 0, 1]);
            const response = await request(app)
        ...
        it('debería soportar paginación (limit y offset)', async () => {
            ...
            for (let i = 1; i <= 5; i++) {
                await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', [`Obra Paginada ${i}`, 'Test', 0, 1]);
            }
            const response = await request(app)
        ...
        it('debería filtrar por nombre (LIKE)', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['PAMI Especial', 'Test', 0, 1]);
            const response = await request(app)
        ...
        it('debería ordenar por nombre descendente', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['AAA', 'Test', 0, 1]);
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['ZZZ', 'Test', 0, 1]);
            const response = await request(app)
        ...
        it('debería retornar solo obras sociales activas por defecto', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Activa Defecto', 'Test', 0.1, 1]);
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Inactiva Defecto', 'Test', 0.1, 0]);
            const response = await request(app)
                .get('/api/v1/obras-sociales')
                .set('Authorization', `Bearer ${adminToken}`);
            expect(response.status).toBe(200);
        ...
        it('debería retornar solo obras sociales inactivas cuando activo=0', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Activa Filtro', 'Test', 0.1, 1]);
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Inactiva Filtro', 'Test', 0.1, 0]);
            const response = await request(app)
        ...
        it('debería retornar todas las obras sociales (activas e inactivas) cuando activo=all', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Activa All', 'Test', 0.1, 1]);
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Inactiva All', 'Test', 0.1, 0]);
            const response = await request(app)
        ...
        it('debería filtrar por nombre y estado activo=0 simultáneamente', async () => {
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['PAMI Activa', 'Test', 0, 1]);
            await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['PAMI Inactiva', 'Test', 0, 0]);
            const response = await request(app)
    ...
    describe('GET /api/v1/obras-sociales/:id', () => {
        it('debería retornar los datos de una obra social activa', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Obra Por ID', 'Test', 0, 1]);
            const id = result.insertId;
            const response = await request(app)
        ...
        it('debería retornar 404 si la obra social está inactiva', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Obra Inactiva ID', 'Test', 0, 0]);
            const id = result.insertId;
            const response = await request(app)
                .get(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`);
        ...
        it('debería retornar la estructura DTO camelCase completa para una obra social', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, es_particular, activo) VALUES (?, ?, ?, ?, ?)', ['DTO Completo', 'Plan Full', 0.2, 1, 1]);
            const id = result.insertId;
            const response = await request(app)
                .get(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`);
    ...
    describe('DELETE /api/v1/obras-sociales/:id', () => {
        it('debería realizar un borrado lógico (activo = 0) en la base de datos', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Para Borrar', 'Test', 0, 1]);
            const id = result.insertId;
            const response = await request(app)
        ...
        it('debería retornar 404 al intentar eliminar una obra social ya inactiva', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Ya Inactiva', 'Test', 0, 0]);
            const id = result.insertId;
            const response = await request(app)
                .delete(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`);
        ...
        it('debería realizar un soft delete y no un hard delete (el registro permanece en la base de datos)', async () => {
            const [result] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['Soft Delete Test', 'Test', 0, 1]);
            const id = result.insertId;
            const response = await request(app)
                .delete(`/api/v1/obras-sociales/${id}`)
                .set('Authorization', `Bearer ${adminToken}`);
        ...
        it('debería permitir soft delete incluso si está vinculada a un paciente (integridad referencial de BD intacta)', async () => {
            ...
            const [osResult] = await pool.execute('INSERT INTO obras_sociales (nombre, descripcion, porcentaje_descuento, activo) VALUES (?, ?, ?, ?)', ['OS Vinculada', 'Para Paciente', 0.1, 1]);
            const idObraSocial = osResult.insertId;
            // 2. Insertamos usuario para el paciente
            const [userResult] = await pool.execute('INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, SHA2(?, 256), ?, ?, ?)', ['55555555', 'Paciente', 'Test', 'paciente@test.com', 'password', '', ROLES.PACIENTE, 1]);
            ...
            const [rows] = await pool.execute('SELECT activo FROM obras_sociales WHERE id_obra_social = ?', [idObraSocial]);
            expect(rows[0].activo).toBe(0);
        });
    });
