...
import * as pacientesModel from '../../src/database/pacientes.ts';
import * as obrasSocialesModel from '../../src/database/obras_sociales.ts';
import { ATTENDED_STATUS } from '../../src/constants/common.constants.ts';
...
describe('Turnos Service - Unit Tests', () => {
    ...
    describe('getMyAppointments()', () => {
        ...
        it('debería lanzar NOT_FOUND si el médico no tiene perfil', async () => {
            ...
            await expect(turnosService.getMyAppointments({ id: 99, rol: ROLES.MEDICO })).rejects.toMatchObject({
                code: 'NOT_FOUND',
        ...
        it('debería lanzar NOT_FOUND si el paciente no tiene perfil', async () => {
            ...
            await expect(turnosService.getMyAppointments({ id: 99, rol: ROLES.PACIENTE })).rejects.toMatchObject({
                code: 'NOT_FOUND',
    ...
    describe('createAppointment() - Calculation Logic', () => {
        it('debería calcular el valorTotal correctamente para una obra social con descuento (esParticular = false)', async () => {
            ...
            expect(turnosModel.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('createAppointment() - Identity Resolution (PACIENTE)', () => {
        ...
        it('debería lanzar NOT_FOUND si el paciente no tiene perfil al intentar registrar un turno', async () => {
            pacientesModel.findByUserId.mockResolvedValue(null);
            await expect(turnosService.createAppointment({}, { id: 3, role: ROLES.PACIENTE })).rejects.toMatchObject({
                code: 'NOT_FOUND',
                message: 'Perfil de paciente no encontrado',
            });
        });
    ...
    describe('createAppointment() - Validations', () => {
        it('debería lanzar error si el médico no existe', async () => {
            ...
            await expect(turnosService.createAppointment({ idMedico: 999 }, { id: 1, role: ROLES.ADMIN })).rejects.toThrow('El médico solicitado no existe');
        });
        it('debería lanzar error si el médico no está activo', async () => {
            medicosModel.findById.mockResolvedValue({ idMedico: 1, activo: false });
            await expect(turnosService.createAppointment({ idMedico: 1 }, { id: 1, role: ROLES.ADMIN })).rejects.toMatchObject({
                status: 422,
        ...
        it('debería lanzar error si el paciente no existe', async () => {
            ...
            await expect(turnosService.createAppointment({ idMedico: 1, idPaciente: 999 }, { id: 1, role: ROLES.ADMIN })).rejects.toThrow('El paciente solicitado no existe');
        });
        it('debería lanzar error si el paciente no está activo', async () => {
            ...
            await expect(turnosService.createAppointment({ idMedico: 1, idPaciente: 1, idObraSocial: 1 }, { id: 1, role: ROLES.ADMIN })).rejects.toMatchObject({
                status: 422,
                code: 'VALIDATION_ERROR',
        ...
        it('debería lanzar error si la obra social del turno no coincide con la del paciente y no es particular', async () => {
            ...
            await expect(turnosService.createAppointment({ idMedico: 1, idPaciente: 1, idObraSocial: 2 }, { id: 1, role: ROLES.ADMIN })).rejects.toMatchObject({
                status: 422,
                code: 'VALIDATION_ERROR',
        ...
        it('debería lanzar error si la obra social no existe', async () => {
            ...
            await expect(turnosService.createAppointment({ idMedico: 1, idPaciente: 1, idObraSocial: 999 }, { id: 1, role: ROLES.ADMIN })).rejects.toThrow('La obra social solicitada no existe');
        });
        ...
        it('debería lanzar error si el médico ya tiene un turno reservado para la misma fecha y hora', async () => {
            ...
            await expect(turnosService.createAppointment(data, { id: 1, role: ROLES.ADMIN })).rejects.toMatchObject({
                status: 409,
        ...
        it('debería lanzar DUPLICATE_ENTRY si el modelo retorna null (conflicto atómico de concurrencia)', async () => {
            ...
            await expect(turnosService.createAppointment(data, { id: 1, role: ROLES.ADMIN })).rejects.toMatchObject({
                status: 409,
                code: 'DUPLICATE_ENTRY',
                message: 'No se pudo reservar el turno. El médico o el paciente ya tienen un compromiso en ese horario.',
            });
        });
    });
