...
describe('Turnos List - Integration Tests', () => {
    ...
    beforeEach(async () => {
        ...
        // 3. Usuarios
        await pool.execute('INSERT INTO usuarios (id_usuario, documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [1, '11111111', 'Admin', 'Root', 'admin@test.com', 'hash', '', ROLES.ADMIN, 1]);
        ...
        // 6. Turno
        await pool.execute('INSERT INTO turnos_reservas (id_medico, id_paciente, id_obra_social, fecha_hora, valor_total, atendido, activo) VALUES (?, ?, ?, ?, ?, ?, ?)', [1, 1, 1, '2026-07-15 14:30:00', 4500.0, 0, 1]);
        // Tokens
    ...
    describe('GET /api/v1/turnos', () => {
        ...
        it('Doctor without profile should return 404', async () => {
            ...
            await pool.execute('INSERT INTO usuarios (id_usuario, documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [4, '44444444', 'Ghost', 'Doc', 'ghost@test.com', 'hash', '', ROLES.MEDICO, 1]);
        ...
        it('Patient without profile should return 404', async () => {
            ...
            await pool.execute('INSERT INTO usuarios (id_usuario, documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [5, '55555555', 'Ghost', 'Patient', 'ghostpatient@test.com', 'hash', '', ROLES.PACIENTE, 1]);
            const ghostToken = jwt.sign({ id: 5, rol: ROLES.PACIENTE, documento: '55555555' }, JWT_SECRET);
            const response = await request(app)
                .get('/api/v1/turnos')
                .set('Authorization', `Bearer ${ghostToken}`);
            expect(response.status).toBe(404);
        ...
        it('Should not list inactive turnos', async () => {
            ...
            await pool.execute('INSERT INTO turnos_reservas (id_medico, id_paciente, id_obra_social, fecha_hora, valor_total, atendido, activo) VALUES (?, ?, ?, ?, ?, ?, ?)', [1, 1, 1, '2026-07-16 10:00:00', 4500.0, 0, 0]);
            const response = await request(app)
                .get('/api/v1/turnos')
                .set('Authorization', `Bearer ${patientToken}`);
            expect(response.status).toBe(200);
        ...
        it('Should return turnos ordered by date descending', async () => {
            ...
            await pool.execute('INSERT INTO turnos_reservas (id_medico, id_paciente, id_obra_social, fecha_hora, valor_total, atendido, activo) VALUES (?, ?, ?, ?, ?, ?, ?)', [1, 1, 1, '2026-07-10 09:00:00', 4500.0, 0, 1]);
            const response = await request(app)
                .get('/api/v1/turnos')
                .set('Authorization', `Bearer ${patientToken}`);
            expect(response.status).toBe(200);
        ...
        it('Should return an empty array if no turnos found', async () => {
            ...
            await pool.execute('INSERT INTO usuarios (id_usuario, documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [6, '66666666', 'New', 'Patient', 'new@test.com', 'hash', '', ROLES.PACIENTE, 1]);
            await pool.execute('INSERT INTO pacientes (id_paciente, id_usuario, id_obra_social) VALUES (?, ?, ?)', [2, 6, 1]);
        ...
        it('Should filter by "atendido" status', async () => {
            ...
            await pool.execute('INSERT INTO turnos_reservas (id_medico, id_paciente, id_obra_social, fecha_hora, valor_total, atendido, activo) VALUES (?, ?, ?, ?, ?, ?, ?)', [1, 1, 1, '2026-06-01 10:00:00', 4500.0, 1, 1]);
        ...
        it('Should support custom sorting via "order" and "asc" params', async () => {
            ...
            await pool.execute('INSERT INTO turnos_reservas (id_medico, id_paciente, id_obra_social, fecha_hora, valor_total, atendido, activo) VALUES (?, ?, ?, ?, ?, ?, ?)', [1, 1, 1, '2026-01-01 08:00:00', 4500.0, 0, 1]);
