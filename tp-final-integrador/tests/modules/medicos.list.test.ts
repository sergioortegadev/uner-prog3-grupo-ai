...
describe('Médicos Listado - Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const [pResult] = await pool.execute(
      ...
      ['123', 'Patient', 'P', 'p@test.com', 'hash', '', ROLES.PACIENTE, 1],
    );
    patientToken = jwt.sign(
      { id: pResult.insertId, rol: ROLES.PACIENTE, documento: '123' },
      JWT_SECRET,
    );

    const [aResult] = await pool.execute(
      'INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      ['456', 'Admin', 'A', 'a@test.com', 'hash', '', ROLES.ADMIN, 1],
    );
    adminToken = jwt.sign({ id: aResult.insertId, rol: ROLES.ADMIN, documento: '456' }, JWT_SECRET);

    const [dResult] = await pool.execute(
      'INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      ['789', 'Doc', 'D', 'd@test.com', 'hash', '', ROLES.MEDICO, 1],
    );
    doctorToken = jwt.sign(
      { id: dResult.insertId, rol: ROLES.MEDICO, documento: '789' },
      JWT_SECRET,
    );
  });
  ...
  describe('GET /api/v1/medicos', () => {
    it('should return 200 and list of active doctors for patient', async () => {
      ...
      const [u1] = await pool.execute(
        'INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['111', 'Active', 'Doc', 'active@test.com', 'hash', '', ROLES.MEDICO, 1],
      );
      await pool.execute(
        'INSERT INTO medicos (id_usuario, id_especialidad, matricula, valor_consulta) VALUES (?, ?, ?, ?)',
        [u1.insertId, espId, 111, 1000],
      );

      // 2. Create Inactive Doctor
      const [u2] = await pool.execute(
        'INSERT INTO usuarios (documento, apellido, nombres, email, contrasenia, foto_path, rol, activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['222', 'Inactive', 'Doc', 'inactive@test.com', 'hash', '', ROLES.MEDICO, 0],
      );
      await pool.execute(
        'INSERT INTO medicos (id_usuario, id_especialidad, matricula, valor_consulta) VALUES (?, ?, ?, ?)',
        [u2.insertId, espId, 222, 2000],
      );
